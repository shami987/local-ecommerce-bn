...
const createShop = async (req, res) => {
    try {
        ...
        const shop = await Shop_1.ShopModel.create({ name, description, location, telephone, email, image });
...
const updateShop = async (req, res) => {
    try {
        const { name, description, location, telephone, email, image } = req.body;
        const shop = await Shop_1.ShopModel.findByIdAndUpdate(req.params.id, { name, description, location, telephone, email, image }, { new: true, runValidators: true });
        if (!shop) {
            return res.status(404).json({ message: 'Shop not found' });
        }
